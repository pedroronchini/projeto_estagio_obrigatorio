...
def main(genomas, config):
    ...
    while rodando:
            ...
            if not ai_jogando:
                if evento.type == pygame.KEYDOWN:
                    if evento.key == pygame.K_SPACE:
                        for passaro in passaros:
                            passaro.pular()
        # Para fazer a inteligência artificial verificar sempre o proxiomo cano
        indice_cano = 0
        ...
        for i, passaro in enumerate(passaros):
            passaro.mover()
            # aumenta o fitnes, que vai ser responsável para descobrir qual é o melhor passaro de cada geração
            lista_genomas[i].fitness += 0.1
            output = redes[i].activate((passaro.y, abs(passaro.y - canos[indice_cano].altura), abs(passaro.y - canos[indice_cano].posicao_base)))
